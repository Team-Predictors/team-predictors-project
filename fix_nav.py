...
nav_template = '''    <!-- UNIFIED BIG NAVBAR -->
    <nav class="w-full h-[105px] px-8 lg:px-20 flex items-center justify-between bg-white border-b border-gray-100 z-50 relative shrink-0">
        ...
        <div class="hidden lg:flex items-center gap-10 text-[14px] font-bold text-gray-700" style="font-family: 'Inter', sans-serif;">
            <a href="index.html" class="{index_class} hover:text-green-700 transition">Home</a>
            <a href="about.html" class="{about_class} hover:text-green-700 transition">About</a>
            <a href="features.html" class="{features_class} hover:text-green-700 transition">Features</a>
            <a href="how_it_works.html" class="{how_class} hover:text-green-700 transition">How It Works</a>
            <a href="contact.html" class="{contact_class} hover:text-green-700 transition">Contact</a>
        </div>
...
for f in files:
    ...
    # Active class logic
    idx = abt = feat = how = cnt = ""
    if f == 'index.html': idx = "text-green-700 border-b-[3px] border-green-700 pb-1"
    elif f == 'about.html': abt = "text-green-700 border-b-[3px] border-green-700 pb-1"
    elif f == 'features.html': feat = "text-green-700 border-b-[3px] border-green-700 pb-1"
    elif f == 'how_it_works.html': how = "text-green-700 border-b-[3px] border-green-700 pb-1"
    elif f == 'contact.html': cnt = "text-green-700 border-b-[3px] border-green-700 pb-1"
    
    new_nav = nav_template.format(index_class=idx, about_class=abt, features_class=feat, how_class=how, contact_class=cnt)
    
    # Replace the existing nav tag and its contents
    # ChatGPT added <nav class="..."> ... </nav> in all files.
